...
namespace KalOS
{
    ...
    public partial class App : Application
    {
        ...
        private void ConfigureServices(ServiceCollection services)
        {
            ...
            services.AddSingleton<PrUpdateService>();
        ...
        /// <summary>
        /// Dev builds only: checks GitHub for open contributor PRs once at
        /// startup and pops a dialog offering to review/merge them — the same
        /// UX the update check has. Consumer builds never call this; PRs are a
        /// toolkit-only feature.
        /// </summary>
        private void StartPrCheck()
        {
#if !CONSUMER_BUILD
            var settingsVm = Services.GetRequiredService<SettingsViewModel>();
            settingsVm.PrsDetectedAtStartup += OnPrsDetectedAtStartup;

            // Same pattern as StartUpdateCheck: wait for the window to be
            // visible, then run the check ON THE UI THREAD (the view model
            // raises PropertyChanged, which x:Bind requires).
            var queue = _window?.DispatcherQueue;
            _ = Task.Run(async () =>
            {
                ...
                queue.TryEnqueue(() =>
                {
                    try { settingsVm.RunStartupPrCheck(); }
                    catch { /* never crash on PR plumbing */ }
                });
            });
#endif
        }

        /// <summary>Startup PR check found open pull requests — ask the user what to do.</summary>
        private void OnPrsDetectedAtStartup(IReadOnlyList<PrInfo> prs)
        {
            ...
            _window.DispatcherQueue.TryEnqueue(async () =>
            {
                try
                {
                    if (_window?.Content is not FrameworkElement inner || inner.XamlRoot == null) return;
                    var first = prs[0];
                    var more = prs.Count > 1 ? $"\n(and {prs.Count - 1} more)" : string.Empty;
                    var dialog = new ContentDialog
                    {
                        Title = $"{prs.Count} contributor pull request{(prs.Count == 1 ? "" : "s")} found",
                        Content = new TextBlock
                        {
                            Text = $"#{first.Number} — {first.Title} (by {first.Author}){more}\n\nReview the files it changes and merge them into this source tree (backed up and undoable) — then rebuild and reopen.",
                            TextWrapping = TextWrapping.Wrap,
                            MaxWidth = 420
                        },
                        PrimaryButtonText = "Review in Settings",
                        CloseButtonText = "Later",
                        DefaultButton = ContentDialogButton.Primary,
                        XamlRoot = inner.XamlRoot
                    };
                    TrackDialog(dialog);
                    if (await dialog.ShowAsync() == ContentDialogResult.Primary && _window is MainWindow main)
                    {
                        // Pre-select the first PR so the Settings page arrives
                        // with its changed-file list already loading — the user
                        // lands on actionable content, not an empty card.
                        settingsVm.PendingFocusPr = first;
                        main.NavigateToPage(typeof(KalOS.Views.SettingsPage));
                    }
                }
                catch
                {
                    // Never let the PR popup crash the app.
                }
            });
        }
