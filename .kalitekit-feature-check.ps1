...
$dll = 'D:\KaliteKit-consumer\KaliteKit.dll'
...
if (Test-Path "$env:LOCALAPPDATA\KaliteKit\updates\update.log") { Get-Content "$env:LOCALAPPDATA\KaliteKit\updates\update.log" } else { Write-Output "not present" }
Write-Output ""
Write-Output "=== release state (anonymous, what consumer sees) ==="
try {
    $r = Invoke-RestMethod -Uri 'https://api.github.com/repos/1k09-byte/KaliteKit/releases/latest' -Headers @{ 'User-Agent' = 'KaliteKit-check' } -TimeoutSec 20
